...
class PatchTransformer(nn.Module):
    ...
    def forward(self,
        ...
        for lab in lab_batch:
            ...
            new_lab[:,
                    0] = lab[:, 0] + (lab[:, 2] - lab[:, 0]) / 2
            new_lab[:,
                    1] = lab[:, 1] + (lab[:, 3] - lab[:, 1]) / 2 + 420  # 1920

            # new_lab[:,
            #         0] = lab[:, 0] + (lab[:, 2] - lab[:, 0]) / 2 - 100
            # new_lab[:,
            #         1] = lab[:, 1] + (lab[:, 3] - lab[:, 1]) / 2 - 100  # 640
            
            new_lab[:, 2] = lab[:, 2] - lab[:, 0]
            new_lab[:, 3] = lab[:, 3] - lab[:, 1]
            new_lab = new_lab / img_size
        ...
        lab_batch_scaled[:, :, 0] = lab_batch[:, :, 0] * img_size
        lab_batch_scaled[:, :, 1] = lab_batch[:, :, 1] * img_size
        lab_batch_scaled[:, :, 2] = lab_batch[:, :, 2] * img_size
        lab_batch_scaled[:, :, 3] = lab_batch[:, :, 3] * img_size

        target_size = torch.sqrt(((lab_batch_scaled[:, :, 2].mul(0.2))**2) +
        ...
        if (rand_loc):
            off_x = targetoff_x * (
                torch.FloatTensor(targetoff_x.size()).uniform_(-0.4, 0.4))
            target_x = target_x + off_x
            off_y = targetoff_y * (
                torch.FloatTensor(targetoff_y.size()).uniform_(-0.4, 0.4))
