...
class OBJYOLODetector(YOLODetector):
    ...
    #             # objectness_i = objectness_i.reshape(batch, -1)
    #             # normal_confs = torch.nn.Softmax(dim=1)(cls_scores_i)
    #             confs_if_object = cls_scores_i * objectness_i
    #             max_conf, max_conf_idx = torch.topk(confs_if_object, k=10, dim=1)
    #             obj_loss = torch.cat([obj_loss, max_conf.sum(dim=1)], dim=0)
    ...
    #             # print(cls_scores_i.shape)
    #             top10pecent = int(cls_scores_i.shape[1] * 0.1)
    #             max_conf, max_conf_idx = torch.topk(cls_scores_i, k=top10pecent, dim=1)
    #             # max_conf, max_conf_idx = torch.topk(cls_scores_i, k=10, dim=1)
    #             # max_conf, max_conf_idx = torch.max(cls_scores_i, dim=1)
    #             obj_loss = torch.cat([obj_loss, max_conf.mean(dim=1)], dim=0)
    #         obj_loss = obj_loss.max()
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if len(results.scores) != 0:
            obj_loss = results.scores.mean()
        else:
            obj_loss = torch.Tensor([0.]).cuda().requires_grad_(True)
    ...
    def predict(self,
        ...
        if len(gt_bboxes[0]) != 0:
            ...
            img = img[:, pad_pixel:img.shape[-1] - pad_pixel, :] # 去掉灰边
